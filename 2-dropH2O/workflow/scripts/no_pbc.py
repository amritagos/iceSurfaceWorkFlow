...
def shift_admolecule(admolecule_idx: int, atoms: Atoms) -> np.ndarray:
...
def main(
    ...
    natoms = len(system)
    dropped_o_index = natoms - 3

    # Set PBCs to false and put admolecule in center
    system.pbc = [False, False, False]
    displacement = shift_admolecule(dropped_o_index, system)
    np.save(out_metadata.parent / "displacement.npy", displacement)
    ...
    results["system_energy"] = energy
    results["dipole_moment"] = dipole_moment_magnitude
